...
| `gwas/gwas-catalog-studies.tsv` | GWAS Catalog — studies | 18.5 MB | 120,064 published genome-wide association studies with trait, sample description and association count. The psychiatric traits are largely Psychiatric Genomics Consortium output. | EMBL-EBI terms; freely available |
| `gwas-catalog-associations.zip` | GWAS Catalog — associations with mapped genes | EMBL-EBI terms of use; freely available | 70 MB | Every catalogued association with the gene its variant was **mapped to**. The studies file says which traits were studied; this says which genes came out. Ingested for `tools/addiction_cells.py`: "which cell types does the genetics of addiction implicate" cannot be asked without a gene column. |
| `obesity/TF150_ThermoScores_cell.csv` | Broad / EWSC obesity challenge — thermogenic scores per cell | 12.6 MB | 25,296 cells with twelve signature z-scores and the perturbation each received. Carries a DESIGNED non-targeting control 2,242 cells deep — option 1 of the three control pools the adapter skill ranks, and the first time this repository has had one. | challenge data; **not redistributable** |
| `obesity/TF150_ThermoScores_perturbation.csv` | Broad / EWSC obesity challenge — per perturbation | 0.03 MB | The aggregate the challenge scores on, including its own `agg_top3_z`, so the calibration can be read against the competition's own number. | challenge data; **not redistributable** |
| `gwas/gwas-catalog-studies-accessions.txt` | GWAS Catalog — studies with accession and mapped trait | 8.6 MB | THE JOIN THAT MAKES THE ANCESTRY FILE USABLE. The studies file is keyed on PubMed id, and one phenome-wide paper can carry over a thousand accessions across unrelated traits — selecting papers by trait imports the whole phenome. This carries STUDY ACCESSION beside the mapped MONDO term. | EMBL-EBI terms; freely available |
| `gwas/gwas-catalog-ancestry.tsv` | GWAS Catalog — ancestry | 51.6 MB | WHO WAS ACTUALLY IN THE SAMPLE, per study and stage: broad ancestral category, number of individuals, country of origin and of recruitment. Turns the ancestry caveat several results here carry into a count. | EMBL-EBI terms; freely available |
| `gwas/gwas-efo-trait-mappings.tsv` | GWAS Catalog — trait to EFO/MONDO mappings | 37 MB | Each study's free-text trait mapped to an ontology term, 2,897 of them MONDO — the identifier space the rare atlas already runs on. | EMBL-EBI terms; freely available |
