...
| `psychiatric_gwas.py` | 260 | GWAS Catalog studies, accessions and ancestry | who was in the sample on the disorders human genetics is most confident about: **65.8 %** of analysis weight is European, 11.9 % states no ancestry at all, and **4 of 9 disorders have zero** analyses with an African-ancestry majority. Two joins had to be fixed first — a paper-level join imported a phenome-wide study whole, and a naive sum returned 4.4 billion people |
| `addiction_cells.py` | 300 | which cells substance-use genetics lands in, against a null matched on EXPRESSION BREADTH. Disorder and quantity phenotypes share a **median of zero** cell types: for nicotine, dependence points at the adrenal-pituitary stress axis and consumption at cortical neurons |
| `addiction_atlas.py` | 300 | substance use, entered through what the field's headings hide: only **26 %** [19, 33] of the sample behind substance-use GWAS measures a disorder. Nicotine **13 %**. Stimulants have 29,153 sample against alcohol's 44.5 M. 200 accessions excluded as not about the substance |
| `trait_atlas.py` | 330 | GWAS Catalog + EFO parent terms | every disease area on the same axes, and the comparison **inverts the psychiatric claim**: psychiatry is the LEAST European of the eight areas, not the most — spread 17.7 points from Psychiatric disorder (authored) to Other disease. Emits a parallel-coordinates model and a doubly seriated matrix, both solved in Python per ADR 0008 |
| `signal_energy.py` | 330 | HPO, Reactome top level | **A NEGATIVE RESULT AGAINST THIS REPOSITORY'S OWN READING.** scale_information concludes that a pathway alphabet fails on FORM because it has no vocabulary for space; that treats 29 pathways as one alphabet. Used one at a time as single bits, the field families (signalling, development, ECM) do **not** lean towards morphogenetic systems more than the energy families do — **-0.009477 against -0.005272** bits, and every family leans the same way. The entropy objection was tested and does not rescue it (**-0.003899** against **-0.002591** normalised). On this evidence the loss is about how coarse the alphabet is, not about what its letters mean. |
...
| `community_identity.py` | 300 | what each community IS, from Reactome — outside the loop that built the graph — against an annotation-matched null. 22 of 28 testable communities get a name; "Sensory processing of sound" covers 33 genes against 2.8 expected |
| `relational_primacy.py` | 300 | relations against attributes for predicting disease genes — his most central construct. **+0.287 AUPRC** [+0.239, +0.339], 83.9 % of diseases, +0.297 over a degree-matched rewiring. The first run leaked and returned +0.925 |
| `methods_seed.py` | 260 | the five constructs with their falsifiers and a status field: 2 measured, 2 specified but not run, 1 with no public falsifier |
| `nonreciprocal.py` | 280 | the author's OWN hypothesis — non-reciprocity generates organisation — tested against the falsifier he wrote for it. S_NR = +0.033 [+0.018, +0.051]; a randomised direction gives −0.026, so the wrong direction is worse than none. Neither form of the falsifier triggers |
| `clusterability.py` | 260 | is there anything to cluster? Hopkins, HDBSCAN noise and k-means silhouette against a marginal-preserving shuffle. All three separate — and the shuffle itself scores **0.817** on a statistic whose textbook cutoff is 0.75 |
| `gene_embedding.py` | 290 | a UMAP as an OBJECT UNDER TEST. Trustworthiness 0.945, but 37 % of every gene's neighbours change with the seed, and HDBSCAN calls 75 % of genes noise in the features against 0.01 % on the picture of them |
| `moderated_calibration.py` | 250 | empirical-Bayes moderation (Smyth 2004) pointed at a PERMUTATION null's variance instead of a measurement's. ⚠️ **A negative result**: it cannot fix a near-degenerate null, because the trend itself is the defect. The functions are imported by `twin_propagation.py` |
| `partition_flow.py` | 280 | where three algorithms disagree, gene by gene. Communities matched by `scipy.optimize.linear_sum_assignment` (exact, not greedy); band order by barycentre sweeps, crossings 1,542 → 739. Louvain→Leiden keeps 92.5 % of genes in matched communities, Leiden→label propagation 50.0 % |
| `crispr_matrix.py` | 290 | the whole DepMap screen drawn: 1,178 lines × 17,916 genes = **21.1 M** values, both axes seriated by SVD angle, seam rotated out of the essential band. Column roughness **0.226** seriated against 0.287 alphabetical and 0.348 shuffled |
| `network_layout.py` | 250 | three orderings of the gene graph, seriated in Python (ADR 0008). The spectral one puts 53 % of edges within 1 % of the diagonal without being told where the communities are; a random shuffle reaches 2 % |
| `community_stability.py` | 330 | the published partition against a null and an interval: 12 seeds, 3 algorithms, a resolution sweep, a consensus confidence per gene. First tool here to take a clustering dependency (igraph, leidenalg, scikit-learn) |
