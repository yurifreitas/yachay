...
for (const { name, page, registry } of MIGRATED) {
  ...
  // ALL OR NOTHING ON TIERS. A tier is the band a group sits in, and the rail prints a heading

  // when the tier CHANGES between one group and the next. A group with no tier in a page that

  // uses them does not fall into the band above it — it silently extends whatever band came

  // before, so the reader is told the wrong thing rather than nothing. Partial adoption is

  // the only way this feature breaks, so partial adoption fails the build.

  const groupBlocks = [...pageSrc.matchAll(/\{\s*id:\s*"[a-z_]+",\s*label:[^}]*?question:[^}]*?\}/g)]

    .map((m) => m[0]);

  const tiered = groupBlocks.filter((b) => /tier:/.test(b)).length;

  const untiered = groupBlocks.length - tiered;
  ...
  const tierSplit = tiered > 0 && untiered > 0;

  // A TIER MUST BE CONTIGUOUS. The rail prints a band heading when the tier CHANGES between
  // one group and the next, so a tier that appears, stops, and appears again prints its
  // heading twice and reads as two different bands. This shipped for about a minute on the
  // run dashboard, where `premise` and `state` both sit in "the run" and had three other
  // groups between them.
  const tierSeq = groupBlocks
    .map((b2) => (b2.match(/tier:\s*([A-Za-z][A-Za-z0-9_.]*)/) || [])[1])
    .filter(Boolean);
  const seen = new Set();
  const brokenTiers = [];
  for (let i = 0; i < tierSeq.length; i++) {
    if (tierSeq[i] === tierSeq[i - 1]) continue;
    if (seen.has(tierSeq[i])) brokenTiers.push(tierSeq[i]);
    seen.add(tierSeq[i]);
  }

  if (missing.length || orphan.length || noSub.length || dupNav.length || dupReg.length

      || strayGroup.length || emptyGroup.length || tierSplit
      || brokenTiers.length) {
    ...
    for (const t of brokenTiers) console.error(
      `  tier ${t} is not contiguous — the rail prints a band heading when the tier changes, `
      + `so this one is printed twice and reads as two different bands`);
    if (tierSplit) {

      console.error(`  ${tiered} group(s) declare a tier and ${untiered} do not — an untiered `

        + `group silently extends the band above it, so the reader is told the wrong thing`);
    ...
    console.log(`sections: ${name} — ${regIds.length} sections, every one reachable, `
      ...
      + `${tiered ? ` across ${new Set(groupBlocks.map((b) => (b.match(/tier:\s*([A-Za-z.]+)/) || [])[1])).size} bands` : ""}.`);
