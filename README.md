...
> **Role:** the entry point — what yachay is, what `sieve` does, and where every other
> document lives.
> **Last revised:** 2026-08-29 · **State:** current. The Status section under-declared the
> repository by roughly half until 2026-08-29 (audit A2).
...
Early, and larger than the library alone. **Four of the ten stages** are implemented and
tested in `src/sieve/` (Null, Power, Design, Target), along with the data contracts; the
remaining six are the backlog. They are hardened against the DepMap adapter (~18,000 genes
x ~1,100 cell lines), which is the scale test — a method validated only on the screen that
produced it has not been validated.

Around the library sits the other half of the project, which this section used to omit:
**57 tools, 35 registered pipeline stages and 34 artefacts** in `out/rare/` that build the
rare-disease atlas and the explorer. Audit finding A2 is what caught the omission. That half
is the larger one, and it does **not** yet use the library — audit A15, still open.

**Two things a reader should know before the numbers above.** The obesity figures quoted
further up were measured on a screen that **is not in this repository** and cannot be
reproduced from it. And the NF2 reference application currently reports its positive control
as `NOT RECOVERED`, which by this project's own ADR 0003 means its shortlist is gated shut.
Both are stated here rather than in a footnote because a reader who missed them would
overestimate what has been validated.

### What has actually been measured here, with a null and an interval

Four constructs are governed by [ADR 0007](docs/adr/0007-theory-enters-by-measurement.md),
which says a mathematical idea has no standing until a tool computes it from ingested data:

| result | number |
|---|---|
| what a change of scale costs | 181-fold compression onto 29 Reactome pathways keeps **22 %** of the information genes carry about organ system; onto 154 cell types, **31 %**. Retention spans 5.6-fold across systems, so there is no single right coarse-graining |
| direction | genes predict organ system **2.91×** better than the reverse — and the asymmetry collapses to 1.02 under compression |
| what a reader loses in Portuguese | **42.9 %** of the annotated rare-disease phenotype is readable, with a **69.6-point** spread across organ systems, worst in the nervous system |
| is recorded conflict really disagreement? | **57.2 %** of ClinVar's variant-level conflicts are across-condition only — every condition internally consistent. 48.6 % once panel indications are removed |

These are the only results in the repository that carry a governing ADR, a null, an interval
and a registered drift check. [`docs/references/theory-atlas.md`](docs/references/theory-atlas.md)
grades the ninety proposals they were promoted from.
...
| `tools/README.md` | the 57 scripts grouped by purpose: ingest, catalogue, patient, gene, self-audit, authored, statistics |
